...
- **Replicon-level or gene-level HostPacks in v0.1.** Output is one
  ...
  promoter-strength module (see `rbsforge/docs/MODEL.md`, "What this package
...
1. **Native-RBS survey (classical, no learning).** Once `genome_encoder`
   ...
   `rbsforge/docs/MODEL.md`'s Layer-0/Layer-1 guidance. The survey's mode is a
...
Calculator, not this model alone," `rbsforge/docs/MODEL.md`, Known Limitations).
...
|---|---|
| `beta` | An empirical ΔG→rate conversion fit against real reporter/expression data — not 1/RT, not derivable from sequence (`rbsforge/docs/MODEL.md` makes this point at length for the existing hand-curated HostPacks; it applies with equal force to a learned one). |
...
cited in `rbsforge/docs/MODEL.md`). Effectively every other organism in the world
...
Geobacillus "50K RBS" case already discussed in `rbsforge/docs/MODEL.md`). If the
...
cited in `rbsforge/docs/MODEL.md` remain the best real calibration data that
exists, and are E. coli-only. New organism-specific libraries (what
`rbsforge/docs/MODEL.md` already calls "Layer 2" calibration, in language borrowed
...
- The RBSForge Console's existing badge system (`calibrated` / `prior` /
  `speculative`, `rbsforge/web/rbsforge-console.html`) is the right place to surface
...
3. **(§11 entry 3)** If a single new organism-specific reporter library
   ...
   (Geobacillus, per `rbsforge/docs/MODEL.md`) versus genuinely novel?
